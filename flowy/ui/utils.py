...
def is_ui_running(host: str = "localhost", port: int = 8080) -> bool:
    """Check if the Flowy UI server is running.
    ...
    try:
        ...
        # Check if response is successful and from Flowy
        if response.status == 200:
            data = response.read().decode('utf-8')
            return 'flowy' in data.lower() or 'ok' in data.lower()
        return False
    except Exception:
        return False
...
def get_ui_url(host: str = "localhost", port: int = 8080) -> Optional[str]:
    """Get the URL of the running Flowy UI server.
