...
Modern React-based frontend for the Flowy ML pipeline orchestration framework.
...
Production artifacts will be in the `dist/` directory. The Flowy Python backend automatically serves these files when available.
...
The frontend communicates with the Flowy backend via REST API:
...
**Solution**:
1. Start the backend: `flowy ui start --port 8080`
...
**Solution**: Ensure the `dist/` folder exists in `flowy/ui/frontend/dist/`
