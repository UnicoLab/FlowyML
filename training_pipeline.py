...
def init_model():
    ...
    print(f"   Auto-detected: {model_asset.framework} with {model_asset.parameters:,} params")
...
if __name__ == "__main__":
    ...
    # Remote logging mode: executes locally, logs to remote server via HTTP API
    # os.environ["FLOWYML_EXECUTION_MODE"] = "remote"
    # os.environ["FLOWYML_REMOTE_SERVER_URL"] = "http://localhost:8080/api"

    # Reset config to pick up the new env vars (since flowyml was already imported)
    from flowyml.utils.config import reset_config, get_config
    ...
    print(f"   Server: {config.remote_server_url}")
    print("=" * 70)
