...
class VertexAIOrchestrator(Orchestrator):
    ...
    def _build_worker_pool_specs(
        ...
        container_spec = {
            "image_uri": (docker_config.image if docker_config 
                         else "gcr.io/uniflow/uniflow:latest"),
        ...
        if docker_config and docker_config.env_vars:
            container_spec["env"] = [
                {"name": k, "value": v} 
                for k, v in docker_config.env_vars.items()
            ]
...
class GCRContainerRegistry(ContainerRegistry):
    ...
    def __init__(
        ...
        region: Optional[str] = None,  # For Artifact Registry
    ):
        """
