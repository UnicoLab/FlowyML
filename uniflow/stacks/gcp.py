"""GCP Stack - Google Cloud Platform integration for UniFlow.
...
class VertexAIOrchestrator(Orchestrator):
    ...
    Example:
        ```python
        from uniflow.stacks.gcp import VertexAIOrchestrator
    ...
    def _build_worker_pool_specs(
        ...
        container_spec = {
            "image_uri": (docker_config.image if docker_config else "gcr.io/uniflow/uniflow:latest"),
            "command": ["python", "-m", "uniflow.cli.run"],
        }
...
class GCSArtifactStore(ArtifactStore):
    ...
    Example:
        ```python
        from uniflow.stacks.gcp import GCSArtifactStore

        artifact_store = GCSArtifactStore(bucket_name="my-uniflow-artifacts", project_id="my-gcp-project")
        ```
    """
...
class GCRContainerRegistry(ContainerRegistry):
    ...
    Example:
        ```python
        from uniflow.stacks.gcp import GCRContainerRegistry
...
class GCPStack(Stack):
    """Complete GCP stack for running UniFlow pipelines on Google Cloud Platform.
