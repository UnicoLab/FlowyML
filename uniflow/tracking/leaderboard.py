...
class ModelLeaderboard:
    ...
    def display(self, n: int = 10):
        ...
        if not scores:
            print(f"No entries for metric: {self.metric}")
            return
            
        print(f"\n🏆 Model Leaderboard - {self.metric}")
        print("=" * 80)
        print(f"{'Rank':<6} {'Model Name':<30} {'Score':<15} {'Run ID':<20}")
        print("-" * 80)
        
        for i, score in enumerate(scores, 1):
            print(f"{i:<6} {score.model_name:<30} {score.metric_value:<15.4f} {score.run_id[:18]:<20}")
...
def compare_runs(
    ...
    for metric_name in all_metric_names:
        comparison['metrics'][metric_name] = {
            run_id: comparison['runs'][run_id]['metrics'].get(metric_name)
            for run_id in run_ids
        }
