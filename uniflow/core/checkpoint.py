...
class PipelineCheckpoint:
    ...
    def __init__(
        self,
        run_id: str,
        checkpoint_dir: str = ".uniflow/checkpoints"
    ):
        self.run_id = run_id
    ...
    def clear(self):
        ...
        print(f"🗑️  Checkpoint cleared: {self.run_id}")
...
def checkpoint_enabled_pipeline(pipeline, run_id: str):
    ...
    def run_with_checkpoints(*args, **kwargs):
        ...
        if checkpoint.exists():
            print(f"📍 Checkpoint found. Last completed step: {checkpoint.resume_point()}")
            response = input("Resume from checkpoint? [y/N]: ")
            
            if response.lower() == 'y':
                # Load completed steps
                completed = checkpoint.get_completed_steps()
                print(f"Skipping {len(completed)} completed steps")
                
                # In a real implementation, we would modify the execution
                # to skip completed steps. For now, just notify.
                print("⚠️  Full checkpoint resume not yet implemented.")
                print("   Pipeline will run from the beginning.")
