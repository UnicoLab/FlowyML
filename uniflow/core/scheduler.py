...
class PipelineScheduler:
    ...
    Examples:
        ...
        ...     name="data_refresh",
        ...     pipeline_func=refresh_pipeline,
        ...     hours=6
        ... )
        >>> 
        >>> scheduler.start()
    ...
    def schedule_daily(
        ...
        print(f"📅 Scheduled '{name}' to run daily at {hour:02d}:{minute:02d}")
        return schedule
    ...
    def schedule_interval(
        ...
        self.schedules[name] = schedule
        print(f"⏱️  Scheduled '{name}' to run every {interval_seconds}s")
        return schedule
    ...
    def schedule_hourly(
        ...
        self.schedules[name] = schedule
        print(f"⏰ Scheduled '{name}' to run hourly at :{minute:02d}")
        return schedule
    ...
    def _run_pipeline(self, schedule: Schedule):
        """Run a scheduled pipeline."""
        print(f"\n🚀 Running scheduled pipeline: {schedule.pipeline_name}")
        try:
            result = schedule.pipeline_func()
            schedule.last_run = datetime.now()
            print(f"✅ Completed: {schedule.pipeline_name}")
        except Exception as e:
            print(f"❌ Failed: {schedule.pipeline_name} - {e}")
