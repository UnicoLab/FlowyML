...
TEMPLATES = {
    "basic": {
        "description": "Basic UniFlow project structure",
        "files": {
...
- [UniFlow Documentation](https://docs.uniflow.ai)
- [Examples](https://github.com/uniflow/uniflow/examples)
""",
            "uniflow.yaml": """# UniFlow project configuration
...
def init_project(name: str, template: str, directory: Path) -> None:
    ...
    (directory / ".uniflow").mkdir(exist_ok=True)
    (directory / ".uniflow" / "artifacts").mkdir(exist_ok=True)
    (directory / ".uniflow" / "cache").mkdir(exist_ok=True)
    (directory / ".uniflow" / "runs").mkdir(exist_ok=True)
